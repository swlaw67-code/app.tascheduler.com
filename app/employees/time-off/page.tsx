'use client';
// app/employees/time-off/page.tsx
// Time Off belongs under Employees — mirrors desktop Employees ribbon tab
// This re-exports the existing TimeOff page content with the correct AppShell path
...
interface TOR {
  ...
  date_submitted: string | null; date_requested: string;
  end_date_requested: string | null; reason: string | null;
  all_day: number; start_hour: number; start_minute: number;
  end_hour: number; end_minute: number; approved: number; created_at: string;
}
...
const defaultForm = (): TORForm => ({
  employee_id: 0, date_requested: '', end_date_requested: '',
  reason: '', all_day: true, start_hour: 8, start_minute: 0, end_hour: 17, end_minute: 0,
});
...
export default function TimeOffPage() {
  ...
  async function handleSave() {
    ...
    try {
      ...
      const url    = isEdit ? `/api/time-off/${editTarget!.id}` : '/api/time-off';
  ...
  async function handleApprove(tor: TOR, approved: number) {
    try {
      await fetch(`/api/time-off/${tor.id}`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
  ...
  async function handleDelete() {
    ...
    try {
      await fetch(`/api/time-off/${deleteTarget.id}`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
                      ...
                      <td className="text-sm">
                        ...
                        {tor.end_date_requested && tor.end_date_requested !== tor.date_requested && (
                          <span className="text-[var(--text-muted)]">
                            {' – '}{fmtDate(tor.end_date_requested)}
                          </span>
                        )}
                      </td>
                      <td className="text-sm text-[var(--text-secondary)]">
                        {tor.all_day ? 'All day'
                          : `${formatTime(tor.start_hour, tor.start_minute, false)} – ${formatTime(tor.end_hour, tor.end_minute, false)}`}
                      </td>
                      ...
                      <td>
                        <div className="flex items-center gap-1.5">
                          {tor.approved !== TORStatus.Approved && (
                            <button className="btn btn-sm bg-white border border-green-300
                                               text-green-700 hover:bg-green-50 text-xs"
                          ...
                          <button className="btn-ghost btn-sm text-xs"
                            onClick={() => openEdit(tor)}>Edit</button>
